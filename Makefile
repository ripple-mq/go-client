...
.PHONY: install
## `install`: Fetch dependencies needed to run `go-client`
install:
...
.PHONY: run
## :
## `run`: Run `go-client` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `go-client` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `go-client`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `go-client`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
